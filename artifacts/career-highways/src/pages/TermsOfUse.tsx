...
export default function TermsOfUse() {
  ...
  useEffect(() => {
    document.title = "Terms of Use - Career Highways";
    fetch(`${import.meta.env.BASE_URL}data/site-content.json`)
      .then(res => res.json())
      .then(data => {
        if (data && data["https://careerhighways.com/terms-of-use/"]) {
          setContent(data["https://careerhighways.com/terms-of-use/"].md);
        }
      })
      .catch(err => {
        console.error(err);
        // Fallback text if json is not available at runtime
        setContent("# Career Highways® Terms and Conditions\n\n_Last Updated: October 15, 2025_\n\n**These terms and conditions require individual arbitration to resolve any disputes. Full details regarding individual arbitration and your right to opt out of arbitration are provided below.**\n\n**1. Definitions**\n\n“AI Customer Input” means information, data, materials, text, prompts, images, works, code, or other content that is input, entered, posted, uploaded, submitted, transferred, or otherwise transmitted by or on behalf of you through an AI Feature.");
      });
  }, []);
    ...
    <div className="flex flex-col min-h-screen bg-background">
      <section className="bg-accent text-white pt-32 pb-12">
        <div className="container mx-auto px-4 md:px-6 max-w-4xl text-center">
          <h1 className="font-heading text-5xl md:text-6xl uppercase">Terms of Use</h1>
        </div>
      </section>
